...
class RestaurantController extends Controller
{
    ...
    public function getDeliveryRestaurants(Request $request)
    {
        ...
        if (Cache::has('stores-delivery-active')) {
            ...
            // START OF SORTED RESTO
            $sf = 3.14159 / 180; // scaling factor
            $er = 6350; // earth radius in miles, approximate
            $mr = 100; // max radius
            $lon = $request->longitude;
            $lat = $request->latitude;
            $queryA = "SELECT * FROM restaurants WHERE is_accepted = 1 && is_active = 1 ORDER BY ACOS(SIN(latitude*$sf)*SIN($lat*$sf) + COS(latitude*$sf)*COS($lat*$sf)*COS((longitude-$lon)*$sf))";
            //pytagoras
            //(POW((longitude-$request->longitude),2) + POW((latitude-$request->latitude),2))
            
            $restaurants = DB::select($queryA);
            $restaurants = Restaurant::hydrate($restaurants);
            // END OF SORTED RESTO by ACE
            $this->processSuperCache('stores-delivery-active', $restaurants);
        }
